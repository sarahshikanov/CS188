...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[PAC] = list(set(possible_positions))
    variableDomainsDict[GHOST0] = list(set(possible_positions))
    variableDomainsDict[GHOST1] = list(set(possible_positions))
    #Observations here are Manhattan distances of Pacman to ghosts ± noise, and are non-negative.
    manDisPacmanGhost0 = []
    for i in range(len(variableDomainsDict[PAC])):
        for j in range(len(variableDomainsDict[GHOST0])):
            manDis = manhattanDistance(variableDomainsDict[PAC][i], variableDomainsDict[GHOST0][j])
            plusmanDis = abs(manDis + MAX_NOISE)
            minusmanDis = abs(manDis - MAX_NOISE)
            manDisPacmanGhost0.append(plusmanDis)            
            manDisPacmanGhost0.append(minusmanDis)
    variableDomainsDict[OBS0] = list(set(manDisPacmanGhost0))
    manDisPacmanGhost1 = []
    for i in range(len(variableDomainsDict[PAC])):
        for j in range(len(variableDomainsDict[GHOST1])):
            manDis = manhattanDistance(variableDomainsDict[PAC][i], variableDomainsDict[GHOST1][j])
            plusmanDis = abs(manDis + MAX_NOISE)
            minusmanDis = abs(manDis - MAX_NOISE)
            manDisPacmanGhost1.append(plusmanDis)
            manDisPacmanGhost1.append(minusmanDis)
    variableDomainsDict[OBS1] = list(set(manDisPacmanGhost1))
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # performed the correct interleaving of joins and eliminates.
        #for joinVariable in bayesNet.variablesSet():
        #currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, joinVariable)
        #currentFactorsList.append(joinedFactor
        #in the above, we join everything but we need to incorporate elimination into this 
        #after joinFactorsByVariable, should eliminate factor if condition is met

        #You should join over each variable then eliminate each variable in the elimination order.
        for joinVariable in eliminationOrder:
            #join over each variable
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, joinVariable)
            # If a factor that you are about to eliminate a variable from has 
            #only one unconditioned variable, you should not eliminate it 
            #and instead just discard the factor.  This is since the 
            ...
            #eliminate over each variable considering the condition
        ...
        # currentFactorsList should contain the connected components of the graph now as factors, must join the connected componentsx
        #if we decide not to eliminate a variable (just discard it), it doesn't get added into our factors list for the final join.
        fullJoint = joinFactors(currentFactorsList)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        #d=self(dictionary of keys and values of distributions)
        #k=keys
        factor=1.0/total
        for key in self:
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for possiblePosition in self.allPositions:
            update = self.getObservationProb(observation, gameState.getPacmanPosition(), possiblePosition, self.getJailPosition())
            self.beliefs[possiblePosition] *= update
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos, prob in newPosDist.items():
                #prob is ghost's probability of being in newPos at t+1 given oldPos at t
                dist[newPos] += (prob * self.beliefs[oldPos])
                #update self.beliefs, which reflect places on the board where he believes ghosts are most likely to be given the geometry of the board and ghosts' possible legal moves
        self.beliefs  = dist
        dist.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        for particlePos in range(numberOfParticles):
            self.particles.append(self.legalPositions[particlePos % len(self.legalPositions)])
            #you want self.particles to repeat the list of legal positions, until the size of self.particles is equal to self.num_particles.
            
        #for particlePos in range(self.numParticles % len(self.particles)):
           # self.particles.append(self.legalPositions[particlePos])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        As a reminder, you can obtain Pacman’s position using gameState.getPacmanPosition(),
        ...
        "*** YOUR CODE HERE ***"
        dist = DiscreteDistribution()
        for possiblePosition in self.particles:
            weight = self.getObservationProb(observation, gameState.getPacmanPosition(), possiblePosition, self.getJailPosition())
            dist[possiblePosition] += weight
        if dist.total() == 0:
